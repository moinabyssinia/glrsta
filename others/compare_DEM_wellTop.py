...
top_wells = dat_merged[(dat_merged['layer'] == 1) | (dat_merged['layer'] == '1')]
top_wells['diff'] = top_wells['dem_value1'] - top_wells['top']
print(top_wells)

sns.set_context('paper', font_scale=1.75)
plt.figure()
plt.hist(top_wells['diff'], bins = 300)
plt.title('Difference between ECFTX DEM and Top Elevation of ECFTX wells (ft)')
plt.ylabel('Number of Wells')
plt.xlabel('Difference in feet')

plt.show()

# print(top_wells['diff'].abs().quantile(0.95))

print(len(top_wells[top_wells['diff'] < 0]))
print(len(top_wells[top_wells['diff'] >= 0]))
print((top_wells[top_wells['diff'] <= 0]).quantile(0.05))


neg_diff = top_wells[top_wells['diff'] < 0]

sns.set_context('paper', font_scale=1.75)
plt.figure()
plt.hist(neg_diff['diff'], bins = 300)
plt.title('Number of Wells that have higher elevation than ECFTX DEM')
plt.ylabel('Number of Wells')
plt.xlabel('Difference in feet')
